...
# models.tsv: id <TAB> label <TAB> languages <TAB> modes <TAB> capabilities, lists comma-joined.
#
...
| [ (.id | clean), (.label | clean), (.languages | list), ((.modes // []) | if length > 0 then map(clean) | join(",") else "-" end), ((.capabilities // []) | if length > 0 then map(clean) | join(",") else "-" end) ]
| join("\t")
