...
# (U+2913) on a corpus that is not on this Mac, held off from the title by the gap the picker
# writes, and the squared letters on model labels: M (U+1F13C) MLX, G (U+1F136) ggml,
...
reset_state() {
    ...
    unset FAKE_SPEECH_MODE FAKE_SPEECH_FAIL_FILE FAKE_SPEECH_CATALOG FAKE_SPEECH_DOWNLOAD FAKE_SPEECH_DELETE FAKE_SPEECH_ADD FAKE_SPEECH_EVAL FAKE_SPEECH_STREAM
    omc_reset_controls
}
